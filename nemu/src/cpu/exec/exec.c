...
//    Log(" %0x dest: %s, src: %s val = %0x ", decoding.opcode,decoding.dest.str,decoding.src.str,decoding.src.val);
    e->execute(eip);
}
...
//TODO Error jump code. E contradicts with J.
/* 0x0f 0x01*/
make_group(gp7,
           EMPTY, EMPTY, EMPTY, EMPTY,
           EMPTY, EMPTY, EMPTY, EMPTY)
...
opcode_entry opcode_table[512] = {
        ...
        /* 0x98 */    EMPTY, EX(cltd), IDEX(J, call), EMPTY,
...
//        /* 0xfc */    EMPTY, EMPTY, IDEXW(E, gp4, 1), IDEX(E, gp5),
        ...
        /* 0xb4 */    EMPTY, EMPTY, IDEX(movsx1_E2G, movzx), IDEX(movsx2_E2G, movzx),
        /* 0xb8 */    EMPTY, EMPTY, EMPTY, EMPTY,
        /* 0xbc */    EMPTY, EMPTY, IDEX(movsx1_E2G, movsx), IDEX(movsx2_E2G, movsx),
...
static void print_mem(void){
    int num = 20;
    __uint32_t  addr = 0x7bb0;
    printf("MEM");
    for(int i = 0; i < num; ++i){
        if(i % 5 ==0){
            printf("\n");
        }
        printf("%08x:  %08x    ",addr+i*4,vaddr_read(addr+i*4,4));
...
static void print_reg(void){
    ...
    printf("%s: 0x %08X \n", "eip", cpu.eip);
    printf("%s: 0x %08X \n", "eflags",cpu.eflags);
    printf("CF=%d ZF=%d SF =%d CPU.OF=%d\n",cpu.CF,cpu.ZF,cpu.SF,cpu.OF);
}
static inline void update_eip(void) {

//#ifdef DEBUG
//        Log("Current eip = %0x ", cpu.eip);
//        Log("cpu.is_jump = %d", decoding.is_jmp);
//        Log("cpu.jmp_eip = %0x", decoding.jmp_eip);
//#endif
